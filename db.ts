...
async function migrateToCloud(): Promise<string> {
  ...
  const entities = [
    ...
    { lsKey: 'mg_complaints', sheet: 'Complaints', fields: (item: any) => [item.id, item.bikeNumber || '', item.customerName || '', item.customerPhone || '', item.details || '', (item.photoUrls || []).join(','), item.estimatedCost || 0, item.status || '', item.createdAt || '', item.dueDate || '', item.odometerReading || '', item.assignedMechanicId || '', item.assignedMechanicName || '', item.acceptedAt || '', item.startedAt || '', item.readyAt || '', item.qcApprovedAt || '', item.qcApprovedBy || '', item.deliveredAt || '', JSON.stringify(item.workNotes || []), item.city || ''] },
